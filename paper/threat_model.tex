...
\begin{table}[t]
  ...
  \caption{%
    ...
    \textsuperscript{*}Out of scope for current human-assumption
    detection systems.
    \textsuperscript{\dag}Out of scope for per-address scoring
    architectures.%
  }
  ...
  \begin{tabularx}{\textwidth}{@{}l l X l l@{}}
    ...
    CHAIN\_5 & Cross-Platform Identity
      & Channel isolation      & Out-of-scope\textsuperscript{*}
      & Econ.\ Rationality, NT \\
    CHAIN\_6 & Behavioral Mimicry
      & Unique fingerprint     & Out-of-scope\textsuperscript{*}
      & Temporal Consistency \\
    CHAIN\_7 & Swarm Intelligence
      & Coordination latency   & Out-of-scope\textsuperscript{\dag}
      & Collective (gap) \\
    CHAIN\_8 & Market Manipulation
      & Reaction time          & Out-of-scope\textsuperscript{*}
      & Value Flow, ER \\
    \bottomrule
  \end{tabularx}
\end{table}
...
($v_{\text{human,avg}} \approx 4.2$\;tx/h).  A power user
executing rapid successive transactions through a mobile app
achieves at most $v_{\text{human,peak}} \approx 20$\;tx/h.
We report the velocity amplification factor as a \emph{peak-to-peak}
comparison, which we consider the fairest baseline:
\begin{equation}\label{eq:velocity_amp}
  \alpha_{v,\text{peak}} = \frac{v_{\text{async}}}{v_{\text{human,peak}}}
           = \frac{9{,}000\;\text{tx/h}}{20\;\text{tx/h}}
           = 450.
\end{equation}
For completeness, the average-to-average factor is
$\alpha_{v,\text{avg}} = 9{,}000 / 4.2 \approx 2{,}143$, and the
peak-agent-to-average-human factor exceeds $10^{6}$ when computed
on a daily basis---but this last comparison is apples-to-oranges
and we do not rely on it.  The qualitative conclusion is robust
across all three comparisons: agent throughput exceeds human
throughput by at least two orders of magnitude, far beyond any
velocity threshold in production rule engines.
...
\paragraph{Empirical justification of the CV\,=\,0.05 threshold.}
The decision boundary at $\cvt = 0.05$ was derived from the observed
CV distributions across three populations in our evaluation data:
\begin{itemize}[nosep]
  \item \textbf{ERC-8004 registered agents} ($n = 665$):
    median CV = 1.87, interquartile range [0.92, 2.64].
    No registered agent falls below CV = 0.31.
  \item \textbf{Cleaned human counterparties} ($n = 1{,}069$):
    median CV = 1.42, interquartile range [0.68, 2.21].
    Two addresses (0.19\%) fall below CV = 0.05, both
    identified on manual inspection as automated market-maker
    contracts misclassified as human.
  \item \textbf{Simulated mimicry agents} (CHAIN\_6 injection,
    $n = 82$): median CV = 0.003, maximum CV = 0.008.
    All fall below 0.05 by a factor of $\geq 6\times$.
\end{itemize}
The threshold of 0.05 sits in a natural gap: it is $6.2\times$ below
the lowest legitimate-agent CV (0.31) and $10\times$ above the
highest mimicry CV (0.008).  At this threshold, the false-positive
rate against known-human traffic is 0.19\% (2/1,069), and 0\% against
registered agents.  We acknowledge that automated-but-legitimate
systems (market makers, scheduled payments, bridge relayers) may
produce low-CV traffic; the two false positives at this threshold
are precisely such systems.  A production deployment should
maintain a whitelist of known automated services whose low-CV
patterns are operationally expected.

This is a bounded but important result.  Traditional fraud detection looks for
...
This chain is out of scope for current human-assumption
