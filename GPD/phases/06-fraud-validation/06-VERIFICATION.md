...
verified: 2026-05-03T22:15:54+02:00
status: passed
score: 16/16 contract targets verified
plan_contract_ref: GPD/phases/06-fraud-validation/PLAN.md#/contract
contract_results:
  claims:
    claim-06-injection:
      status: passed
      linked_ids: [deliv-injector-script, at-06-all-chains, ref-06-metrics]
    ...
    claim-06-detection:
      status: passed
      linked_ids: [deliv-validation-script, at-06-recall, at-06-fpr, ref-06-metrics]
    ...
    claim-06-real-world:
      status: passed
      linked_ids: [deliv-fraud-cases, at-06-real-world]
    ...
    claim-06-paper:
      status: passed
      linked_ids: [deliv-arxiv-paper, at-06-paper]
  ...
  deliverables:
    deliv-injector-script:
      ...
      linked_ids: [claim-06-injection, at-06-all-chains]
    ...
    deliv-validation-script:
      ...
      linked_ids: [claim-06-detection, at-06-recall, at-06-fpr, ref-06-metrics]
  ...
  acceptance_tests:
    ...
    at-06-all-chains:
      status: passed
      linked_ids: [claim-06-injection, deliv-injector-script, deliv-validation-script, ref-06-metrics]
    ...
    at-06-paper:
      status: passed
      linked_ids: [claim-06-paper, deliv-arxiv-paper]
      summary: "arXiv paper drafted with complete methodology and results across all 6 phases."
    at-06-real-world:
      status: passed
      linked_ids: [claim-06-real-world, deliv-fraud-cases]
      summary: "No confirmed A2A fraud cases were available; 5 adjacent-domain cases map across the attack taxonomy and preserve the Chain 7 limitation."
  references:
    ref-06-metrics:
      status: completed
      completed_actions: [use, compare]
      missing_actions: []
      summary: "Used data/fraud_detection_metrics.json to compare per-chain recall, ROC-AUC, confusion matrix arithmetic, and false-positive rate against reported values."
  forbidden_proxies:
    fp-06-synthetic-overgeneralization:
      status: rejected
      notes: "The verification preserves the absence of confirmed A2A fraud and treats injection results as validation evidence, not production-ready deployment proof."
    fp-06-chain7-omission:
      status: rejected
      notes: "The Chain 7 0/55 recall result is recorded as a structural limitation rather than hidden inside aggregate metrics."
  uncertainty_markers:
    weakest_anchors:
      - "No confirmed real-world A2A fraud cases were available; Phase 6 validation depends on injected attacks plus adjacent-domain cases."
      - "Chain 7 swarm attacks are structurally missed by the per-address scorer and require group-level detection."
    unvalidated_assumptions:
      - "Injected attack parameters are representative of plausible early agent-economy fraud behavior."
      - "Synthetic 0xdead address isolation does not materially change the graph behavior relevant to signal scoring."
    competing_explanations:
      - "High recall on 7/8 chains may partly reflect synthetic pattern regularity rather than full real-world adversarial diversity."
      - "False-positive performance may be sensitive to benign label noise in the ERC-8004 transaction pool."
    disconfirming_observations:
      - "Chain 7 recall is 0/55 under the current per-address scorer."
      - "No confirmed A2A fraud cases were available for direct real-world validation."
comparison_verdicts:
  - subject_kind: acceptance_test
    subject_id: at-06-recall
    subject_role: decisive
    reference_id: ref-06-metrics
    comparison_kind: benchmark
    verdict: pass
    metric: per_chain_recall
    threshold: ">= 0.90 for 7/8 chains, with Chain 7 structural gap documented"
    recommended_action: "Keep Chain 7 mitigation as future group-level detection work."
    notes: "TP+FN per chain matches metrics JSON. 7 chains at recall=1.0, Chain 7 at recall=0.0."
  - subject_kind: claim
    ...
    verdict: pass
    metric: roc_auc
    threshold: "reported value consistency"
    recommended_action: "Report ROC-AUC alongside the Chain 7 limitation."
    notes: "ROC-AUC 0.777 consistent across primary and operating point thresholds."
  - subject_kind: acceptance_test
    ...
    comparison_kind: benchmark
    verdict: pass
    metric: fpr_operating_point
    threshold: "<= 0.05"
    recommended_action: "Keep threshold 0.29 as the low-FPR operating point."
    notes: "FP=283, TN=7137 at threshold 0.29. FPR = 283/(283+7137) = 0.0381. Matches reported 3.8%."
  - subject_kind: claim
    subject_id: claim-06-detection
    subject_role: supporting
    reference_id: ref-06-metrics
    comparison_kind: experiment
    verdict: pass
    metric: confusion_matrix_primary
    threshold: "row and column sums match address counts"
    recommended_action: "Use the confusion matrix as arithmetic support for the reported recall and FPR."
    notes: "TP=27, FN=55, FP=1617, TN=5803. TP+FN=82 matches attack_addresses. FP+TN=7420 matches benign_addresses."
...
updated: 2026-05-03T22:15:54+02:00
