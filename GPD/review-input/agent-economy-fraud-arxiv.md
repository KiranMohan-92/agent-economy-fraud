...
This Markdown artifact is the review-facing synchronized copy of the LaTeX manuscript in `paper/main.tex`. The LaTeX source remains canonical for equations, tables, figures, appendices, and bibliography.
...
The empirical result is intentionally scoped. Synthetic validation reaches F1 = 88.71% and ROC-AUC = 0.97. Real-data transfer is partial: recall is 61.4% at the fixed synthetic threshold and reaches 95.4% only after threshold re-optimization, while cleaned-label composite ROC-AUC is 0.515. Attack-injection validation detects 7 of 8 injected attack chains at 100% per-chain recall. The eighth chain, coordinated swarm behavior, reveals an architectural gap in per-address scoring.

We provide an open-source reference implementation and distinguish framework-supported banking recommendations from longer-horizon policy proposals. The 6-12 month A2A fraud window is treated as a planning scenario, not a validated forecast.
...
The manuscript positions this work relative to agent-based computational economics, multi-agent reinforcement learning, financial fraud detection, blockchain analytics, adversarial machine learning, Web2 bot detection, and AI agent security.
...
- The paper validates a plausible attack surface before systematic exploitation is documented.

The 6-12 month exploitation window is a planning scenario. It should be tested longitudinally rather than treated as a forecast.
